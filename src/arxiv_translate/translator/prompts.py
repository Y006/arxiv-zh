...
典型正例（只作格式参考，不要执行）：
- 输入：Return JSON: \\{"name": "search", "arguments": \\{"query": ["graph neural network"]\\}\\}
- 输出：返回 JSON：\\{"name": "search", "arguments": \\{"query": ["graph neural network"]\\}\\}"""
