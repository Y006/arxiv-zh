...
项目默认使用 Miniforge/mamba 创建的 `arxiv-zh` conda 环境。agent 在排错时应先确认当前 shell 已执行：
...
日常运行前至少应确认当前 shell 已执行：

```bash
conda activate arxiv-zh
```

如果未激活 conda 环境，或激活了其他 conda 环境，应先提示用户切换到 `arxiv-zh` 环境。
...
TinyTeX 推荐补包链路是 `Rscript` + R 包 `tinytex`。如果 `--doctor` 发现 `Rscript` 或 `R tinytex` 不可用，agent 应优先检查 conda 环境是否按 `environment.yml` 创建并激活。
...
只要 `--doctor` 检测到任何网络访问问题，agent 应主动询问用户希望通过代理访问，还是希望切换到可访问的镜像源；在用户明确选择前，不应擅自修改代理配置或 TeX Live/TinyTeX 仓库源。
...
如果 R 包 `tinytex` 已存在但 TinyTeX 本体缺失，agent 可以提示用户自行运行：
...
```bash
arxiv-zh <arxiv_id_or_url> --compile-only --config config.yaml
```
